...
QUnit.test("test the updateDisplay function ReportBox display", function(assert) {
    updateDisplay(link,[],200);
    assert.equal(rbPass.innerHTML, passed.toString(), "passed innerhtml is set to passed count");
    assert.equal(rbWarning.innerHTML, warning.toString(), "warning innerhtml is set to warning count");
    assert.equal(rbFail.innerHTML, invalid.toString(), "fail innerhtml is set to fail count");
